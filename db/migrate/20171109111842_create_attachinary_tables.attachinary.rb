...
class CreateAttachinaryTables < ActiveRecord::Migration[5.1]
  def change
    create_table :attachinary_files do |t|
      t.references :attachinariable, polymorphic: true, index: {name: 'attch'}
      t.string :scope
    ...
    add_index :attachinary_files, [:attachinariable_type, :attachinariable_id, :scope], {name: 'attchIndex'}
  end
end
